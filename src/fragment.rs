...
pub struct Fragment {
    /// next header identifying the type of the next header
    pub nxt_hdr: u8,
    /// reserved field, should be initialized to all 0
    pub reserved: u8,
    /// first byte of frag_offset field
    pub frag_offset: u8,
    ...
    /// - 1-bit M flag
    pub fo_res_m: u8,
    /// 4 bytes of the packet identification field
    pub id: [u8; 4],
}
...
impl Fragment {
    ...
    const FRAG_OFFSET_UPPER_MASK_IN_BYTE: u8 = 0b11111000; // 0xF8
    const FRAG_OFFSET_UPPER_SHIFT: u8 = 3;
    const FRAG_OFFSET_UPPER_VALUE_MASK: u16 = 0x001F; // For a 5-bit value

    const RESERVED2_MASK_IN_BYTE: u8 = 0b00000110; // 0x06
    const RESERVED2_SHIFT: u8 = 1;
    const RESERVED2_VALUE_MASK: u8 = 0x03; // For a 2-bit value

    const M_FLAG_MASK_IN_BYTE: u8 = 0b00000001; // 0x01
